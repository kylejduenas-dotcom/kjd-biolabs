              ...
              <div className="lg:col-span-2 space-y-4">
                {items.map((item) => (
                  <div key={item.slug} className="flex gap-5 bg-white border border-slate-200/80 rounded-3xl p-4 sm:p-5">
                    <Link href={`/products/${item.slug}`} className="w-24 h-24 rounded-2xl shrink-0 flex items-center justify-center" style={{ background: tintStyles[item.tint].bg }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center justify-between mt-3">
                        ...
                        <span className="text-ink-950 font-display font-bold">{formatPrice(item.price * item.quantity)}</span>
                      </div>
                    </div>
                  </div>
                ))}
