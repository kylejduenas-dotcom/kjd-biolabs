...
export default function Navbar() {
  ...
  const navBg = scrolled
    ? onDark
      ? "bg-[#0a3245]/90 backdrop-blur-xl border-b border-white/10"
  ...
  const ctaCls = onDark
    ? "bg-white text-ink-950 hover:bg-teal-50"
    : "bg-ink-950 text-white hover:bg-teal-600 hover:shadow-lg hover:shadow-teal-500/25";
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${navBg}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            ...
            <button onClick={() => setCartOpen(true)} aria-label="Open cart" className={`relative ml-1 p-2.5 rounded-full transition-colors ${linkCls}`}>
          ...
          <button onClick={() => setMobileOpen(!mobileOpen)} className={`md:hidden p-2 ${onDark ? "text-white" : "text-ink-950"}`} aria-label="Toggle menu">
      ...
      {mobileOpen && (
        <div className={onDark ? "md:hidden bg-[#0a3245] border-t border-white/10" : "md:hidden bg-white border-t border-slate-100"}>
          <div className="px-4 py-4 space-y-1">
            ...
            <MobileLink href={user ? "/account" : "/login"} dark={onDark} onClick={() => setMobileOpen(false)}>{user ? "Account" : "Sign in"}</MobileLink>
            <button onClick={() => { setMobileOpen(false); setCartOpen(true); }} className={`w-full text-left block px-3 py-2.5 text-sm font-medium rounded-lg ${onDark ? "text-white/85 hover:bg-white/10" : "text-slate-700 hover:bg-slate-50"}`}>
              Cart{count > 0 ? ` (${count})` : ""}
            </button>
            <Link href="/products" onClick={() => setMobileOpen(false)} className={`block px-5 py-2.5 rounded-full text-sm font-semibold text-center mt-2 ${onDark ? "bg-white text-ink-950" : "bg-ink-950 text-white"}`}>Browse Catalog</Link>
          </div>
        </div>
      )}
    </nav>
...
function MobileLink({ href, dark, onClick, children }: { href: string; dark: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <Link href={href} onClick={onClick} className={`block px-3 py-2.5 text-sm font-medium rounded-lg ${dark ? "text-white/85 hover:bg-white/10" : "text-slate-700 hover:text-teal-600 hover:bg-slate-50"}`}>
      {children}
    </Link>
  );
}
