        ...
        {dims.map((dim, i) => (
          <button
            ...
            className={`text-left p-4 rounded-2xl border transition-all flex items-center justify-between gap-3 ${
              i === active
                ? "bg-teal-500 border-teal-500 text-white shadow-soft"
                : "bg-white border-slate-200 text-ink-950 hover:border-teal-300"
            }`}
          >
            <span>
              <span className="font-display font-bold block">{dim.key}</span>
              <span className={`text-xs ${i === active ? "text-white/80" : "text-slate-400"}`}>{dim.method}</span>
            </span>
            <svg className={`w-4 h-4 shrink-0 ${i === active ? "text-white" : "text-slate-300"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
