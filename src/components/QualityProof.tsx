    ...
    <svg viewBox="0 0 460 240" className="w-full h-auto" role="img" aria-label="Analysis proof">
      <defs>
        <linearGradient id="qp-area" x1="0" y1="0" x2="0" y2="1">
          <stop offset="0%" stopColor="#2f74c7" stopOpacity="0.45" />
          <stop offset="100%" stopColor="#2f74c7" stopOpacity="0.02" />
        </linearGradient>
      </defs>
      ...
      {type === "peak" && (
        <>
          ...
          />
          <circle cx="168" cy="70" r="3.5" fill="#2360a8" />
        </>
      )}
      {type === "trend" && (
        <>
          <path d="M30 120 L130 118 L230 122 L330 119 L440 121 L440 200 L30 200 Z" fill="url(#qp-area)" />
          <path d="M30 120 L130 118 L230 122 L330 119 L440 121" fill="none" stroke="#2f74c7" strokeWidth="2.5" />
          {[30, 130, 230, 330, 440].map((x, i) => (
            <circle key={i} cx={x} cy={[120, 118, 122, 119, 121][i]} r="3.5" fill="#2360a8" />
          ))}
        </>
      )}
      {type === "flat" && (
        <>
          ...
          <path d="M30 186 L120 184 L210 187 L300 185 L390 186 L440 185" fill="none" stroke="#2f74c7" strokeWidth="2.5" />
        </>
      )}
    </svg>
  );
        ...
        <div className="h-full bg-white rounded-3xl border border-slate-200/80 shadow-soft p-6 sm:p-8">
          <div className="flex items-center justify-between gap-4 mb-4">
            ...
            <div className="text-right">
              <p className="text-2xl font-display font-extrabold text-teal-600">{d.metric}</p>
